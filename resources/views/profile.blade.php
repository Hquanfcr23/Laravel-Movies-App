...
<body>
    <nav class="border-b border-gray-800 bg-gray-900 text-white px-16 md:px-32">
        <div class="container mx-auto flex flex-col md:flex-row items-center justify-between px-4 py-6">
            <ul class="flex flex-col md:flex-row item-center">
                ...
                <li class="ml-8 md:ml-6 mt-3 md:mt-0">
                    <a class="hover:text-gray-300" href="{{route('movies.mylist')}}">My list</a>
                </li>
            ...
            <form action="{{route('profile.update', auth()->user()->id ?? '0')}}" method="POST">
                ...
                <div class="mb-5">
                    <label for="name" class="mb-3 block text-base font-medium text-[#07074D]">
                        User ID
                        <span class="text-gray-600">(read only)</span>
                    </label>
                    <input type="text" value="{{auth()->user()->id ?? 'You are not logged in'}}" readonly name="id" id="id" placeholder="Full Name"
                ...
                <div class="mb-5">
                    ...
                    <input type="text" value="{{auth()->user()->name ?? 'You are not logged in'}}" readonly name="username" id="Username" placeholder="Full Name"
                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-black outline-none focus:border-[#6A64F1] focus:shadow-md" />
                </div>
                <div class="mb-5">
                    <label for="name" class="mb-3 block text-base font-medium text-[#07074D]">
                    ...
                    <input type="text" value="{{auth()->user()->full_name ?? ''}}" name="fullName" id="FullName" placeholder="Full Name"
                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-black outline-none focus:border-[#6A64F1] focus:shadow-md" />
                </div>
                <div class="mb-5">
                    ...
                    <input type="email" value="{{auth()->user()->email ?? 'You are not logged in'}}" name="email" id="email" placeholder="Enter your email"
                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-black outline-none focus:border-[#6A64F1] focus:shadow-md" />
                </div>
                <div class="-mx-3 flex flex-wrap">
                    <div class="w-full px-3 sm:w-1/2">
                        <div class="mb-5">
                            ...
                            <input value="{{auth()->user()->date_of_birth ?? ""}}" type="date" name="dateOfBirth" id="date"
                                class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-black outline-none focus:border-[#6A64F1] focus:shadow-md" />
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-auto w-3/4 py-3 px-6 text-base font-medium text-green-600 outline-none">
